...
class ActionEngine {
    ...
    private func performSystemAction(_ action: String) {
        print("[Action] System action: \(action)")
        switch action {
        case "play_pause":
        ...
        case "show_desktop":
            simulateKeyboardShortcut("cmd+f3")
        case "task_manager":
    ...
    private func simulateMediaKey(keyCode: Int32) {
        // Must send both key-down (0x0A) and key-up (0x0B) — macOS ignores key-down without key-up
        postMediaKeyEvent(keyCode: keyCode, keyDown: true)
        postMediaKeyEvent(keyCode: keyCode, keyDown: false)
        print("[Action] Media key sent: \(keyCode)")
    }

    private func postMediaKeyEvent(keyCode: Int32, keyDown: Bool) {
        ...
        guard let event = NSEvent.otherEvent(
            ...
            print("[Action] Failed to create NSEvent for media key \(keyCode) (down=\(keyDown))")
            return
        }

        guard let cgEvent = event.cgEvent else {
            print("[Action] Failed to get CGEvent from NSEvent for media key \(keyCode) (down=\(keyDown))")
            return
        }
