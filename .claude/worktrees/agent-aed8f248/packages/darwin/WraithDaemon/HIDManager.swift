...
class HIDManager {
    ...
    func start() {
        ...
        // Register callbacks for hot-plug and removal events
        ...
        IOHIDManagerRegisterDeviceRemovalCallback(manager, { context, result, sender, device in
            guard let context = context else { return }
            let mgr = Unmanaged<HIDManager>.fromOpaque(context).takeUnretainedValue()
            mgr.handleDeviceRemoved(device)
        }, selfPtr)

        IOHIDManagerScheduleWithRunLoop(manager, CFRunLoopGetMain(), CFRunLoopMode.defaultMode.rawValue)

        // Check if device is already present at startup
        if let deviceSet = IOHIDManagerCopyDevices(manager) as? Set<IOHIDDevice>, !deviceSet.isEmpty {
            print("[HID] W1 Freestyle found at startup (\(deviceSet.count) interfaces)")
            attachToDevices(deviceSet)
        } else {
            print("[HID] No W1 Freestyle device found. Waiting for plug-in...")
        }
    }

    func stop() {
        cleanupReportBuffers()
    ...
    // MARK: - Hot-plug / removal handlers

    private func handleDeviceConnected(_ device: IOHIDDevice) {
        guard !isConnected else { return }
        print("[HID] W1 Freestyle plugged in")
        if let deviceSet = IOHIDManagerCopyDevices(manager!) as? Set<IOHIDDevice>, !deviceSet.isEmpty {
            attachToDevices(deviceSet)
        }
    }
    ...
    private func handleDeviceRemoved(_ device: IOHIDDevice) {
        guard isConnected else { return }
        print("[HID] W1 Freestyle unplugged")
        cleanupReportBuffers()
        isConnected = false
        state = DeviceState()
        delegate?.hidManagerDidDisconnect()
    }

    // MARK: - Device attachment

    private func attachToDevices(_ deviceSet: Set<IOHIDDevice>) {
        let selfPtr = Unmanaged.passUnretained(self).toOpaque()
    ...
    private func cleanupReportBuffers() {
