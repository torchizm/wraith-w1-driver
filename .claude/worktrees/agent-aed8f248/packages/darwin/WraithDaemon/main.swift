...
class WraithDaemonApp: NSObject, NSApplicationDelegate, HIDManagerDelegate {
    ...
    /// Tracks whether we have seen at least one successful connection.
    /// Used to distinguish initial startup connect from a RE-connect.
    ...
    private func showReconnectNotification() {
        let center = UNUserNotificationCenter.current()
        center.requestAuthorization(options: [.alert, .sound]) { granted, _ in
            guard granted else { return }
            let content = UNMutableNotificationContent()
            content.title = "Wraith W1 Reconnected"
            content.body = "Your mouse is connected and ready."
            content.sound = .default
            let request = UNNotificationRequest(
                identifier: "wraith-reconnect-\(UUID().uuidString)",
                content: content,
                trigger: nil
            )
            center.add(request)
