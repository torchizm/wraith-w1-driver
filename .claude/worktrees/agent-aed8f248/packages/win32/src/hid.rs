                ...
                match open_device() {
                    Ok((dev, cc)) => {
                        ...
                        if has_connected_before {
                            log::info!("[HID] Device reconnected — sending notification");
                        ...
                        *device_arc.lock().unwrap() = None;
                        log::warn!("[HID] Device disconnected — will poll for reconnection");
                        let _ = tx.send(HidEvent::Disconnected);
                    }
                    Err(e) => {
                        log::debug!("[HID] Device not found: {e}. Retrying in 2s…");
...
/// Show a desktop notification when the device reconnects.
fn show_reconnect_notification() {
    // Spawn on a separate thread so we never block the HID read loop.
    std::thread::Builder::new()
        .name("reconnect-notify".into())
        .spawn(|| {
            if let Err(e) = notify_rust::Notification::new()
                .summary("Wraith W1 Reconnected")
                .body("Your mouse is connected and ready.")
                .appname("WraithDaemon")
                .show()
            {
                log::warn!("[HID] Failed to show reconnect notification: {e}");
            }
        })
        .ok();
}
