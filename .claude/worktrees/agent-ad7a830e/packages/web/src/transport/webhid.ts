...
import { parseInputReport, type DeviceState, type ButtonPress } from '../protocol/parser'
import {
  cmdIdentify, parseIdentifyResponse, type IdentifyResult,
  cmdReadPollingRate, cmdReadDPIConfig, cmdReadDPIValue,
  cmdReadSensorParam, cmdReadDebounce, cmdReadSleepTime,
} from '../protocol/commands'
import { POLLING_RATE_MAP, PARAM } from '../protocol/constants'

export interface WebHIDTransport {
  device: Ref<HIDDevice | null>
  connected: Ref<boolean>
  deviceInfo: Ref<IdentifyResult | null>
  state: Ref<DeviceState>
  lastButton: Ref<ButtonPress | null>
  rawHex: Ref<string>
  debugLog: Ref<string[]>
  ...
  readFeatureReport(): Promise<DataView | null>
  sendAndRead(data: Uint8Array, delayMs?: number): Promise<DataView | null>
  freshRead(label: string, cmd: Uint8Array, delayMs?: number): Promise<number[] | null>
  readFullConfig(): Promise<void>
  readDPIConfig(): Promise<void>
  updateStateField(partial: Partial<DeviceState>): void

  onStateUpdate: ((state: DeviceState) => void) | null
  onButtonPress: ((press: ButtonPress) => void) | null
}
...
function dvToBytes(dv: DataView): number[] {
  const bytes: number[] = []
...
export function createWebHIDTransport(): WebHIDTransport {
  ...
  const deviceInfo = ref<IdentifyResult | null>(null)
  const state = ref<DeviceState>({ ...defaultState })
  const lastButton = ref<ButtonPress | null>(null)
  const rawHex = ref('')
  ...
  const transport: WebHIDTransport = {
    device,
    connected,
    deviceInfo,
    state,
    ...
    async connect() {
      ...
      dev.addEventListener('inputreport', (event: HIDInputReportEvent) => {
        ...
        if (parsed.buttonPress) {
          ...
          if (transport.onButtonPress) transport.onButtonPress(parsed.buttonPress)
        }
        ...
        try { localStorage.setItem('wraith:lastState', JSON.stringify(state.value)) } catch {}
        if (transport.onStateUpdate) transport.onStateUpdate(state.value)
      })
     ...
     */
    async freshRead(label: string, cmd: Uint8Array, delayMs = 200): Promise<number[] | null> {
      try {
        // Step 1: Clear stale buffer with a dummy read
        try { await transport.readFeatureReport() } catch {}
        await delay(10)
    ...
    async readFullConfig() {
      ...
      async function readParam(label: string, paramId: number): Promise<number> {
      ...
      if (lodVal >= 0) updates.lodValue = lodVal
      if (angleSnapVal >= 0) updates.angleSnap = angleSnapVal !== 0
      if (motionSyncVal >= 0) updates.motionSync = motionSyncVal !== 0
      if (rippleVal >= 0) updates.rippleEffect = rippleVal !== 0
      if (perfModeVal >= 0) updates.performanceMode = perfModeVal
      if (debounceMs >= 0) updates.debounceTime = debounceMs
      if (sleepSec >= 0) updates.sleepTime = sleepSec
    ...
    updateStateField(partial: Partial<DeviceState>) {
    ...
    disconnect() {
      if (device.value?.opened) device.value.close()
      device.value = null
      connected.value = false
      deviceInfo.value = null
    ...
    async sendFeatureReport(data: Uint8Array) {
      if (!device.value?.opened) throw new Error('Device not connected')
      await device.value.sendFeatureReport(REPORT_ID, data as any)
    },

    async sendReport(reportId: number, data: Uint8Array) {
      if (!device.value?.opened) throw new Error('Device not connected')
      await device.value.sendReport(reportId, data as any)
    },

    async readFeatureReport(): Promise<DataView | null> {
      if (!device.value?.opened) return null
      try {
    ...
    async sendAndRead(data: Uint8Array, delayMs = 200): Promise<DataView | null> {
